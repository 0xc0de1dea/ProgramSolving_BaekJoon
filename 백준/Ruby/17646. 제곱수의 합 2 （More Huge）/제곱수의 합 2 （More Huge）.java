...
class Pollard_rho{
    ...
    public HashMap<Long, Long> factorize(long n){
...
public class Main {
    ...
    public static void print2(long n, long c){
        HashMap<Long, Long> list = new Pollard_rho().factorize(n);
        long[] ans = { 0, 0 };
        long cnt = 1;
        ...
        for (long key : list.keySet()){
            mul *= pow(key, list.get(key) / 2, Long.MAX_VALUE);
        ...
        System.out.print(((ans[0] * cnt * mul) << c) + " " + ((ans[1] * cnt * mul) << c));
    }
