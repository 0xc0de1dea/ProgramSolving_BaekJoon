...
public class Main {
    ...
    public static void main(String[] argu) throws Exception {
        ...
        int n = in.nextInt();
        Square square = new Square(new Line[] { new Line(new Point(-10, -10), new Point(-10, 10)), 
        ...
        jump : for (int i = 0; i < n; i++){
            lines[idx] = new Line(new Point(in.nextInt(), in.nextInt()), new Point(in.nextInt(), in.nextInt()));
            int cnt = 0;

            for (int j = 0; j < 4; j++){
                int tmp = lineSegmentIntersection(lines[idx], square.lines[j]);
                cnt += tmp;
            ...
            if (cnt == 4){
                for (int j = 0; j < idx; j++){
                    if (lineSegmentIntersection(lines[idx], lines[j]) > 0 ){
                        double[] meetPoint = getMeetPoint(lines[idx], lines[j], 2);

                        if (meetPoint[0] <= -10 || meetPoint[0] >= 10 || meetPoint[1] <= -10 || meetPoint[1] >= 10) continue;
