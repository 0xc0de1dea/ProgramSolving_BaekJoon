...
public class Main {
    public static double[] bigMul(double[] a) throws Exception{
        int asz = a.length;
        int shift = (int)Math.ceil(FFT.log2(asz << 1));

        int n = 1; n <<= shift;

        double[] x = new double[n];

        for (int i = 0; i < asz; i++) x[i] = a[i];

        Complex[] X;
        X = FFT.fft(x);

        Complex[] Z = new Complex[n];
        for (int i = 0; i < n; i++) Z[i] = CMath.multiply(X[i], X[i]);
