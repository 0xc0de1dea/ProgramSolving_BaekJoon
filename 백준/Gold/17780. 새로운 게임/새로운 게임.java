...
class ChessPiece {
    ...
    public ChessPiece(int x, int y, int id, int dir){
        this.x = x;
        this.y = y;
        this.id = id;
        this.dir = dir;
...
class BaseData {
    ...
    public BaseData(int destX, int destY, int destSize, int orderType){
        this.destX = destX;
        this.destY = destY;
...
public class Main {
    ...
    public static BaseData getMoveData(ChessPiece base){
        BaseData baseData = new BaseData(0, 0, 0, 2);
    ...
    public static void movePiece(ChessPiece chessPiece, BaseData baseData, int idx){
        if (baseData.orderType == 0){
            ...
            chessPiece.height += baseData.destSize;
        }
        else {
            chessLayer[baseData.destY][baseData.destX].add(chessPiece);
            chessPiece.x = baseData.destX;
            chessPiece.y = baseData.destY;
            chessPiece.height = baseData.currSize + 1 - chessPiece.height;
            chessPiece.height += baseData.destSize;
        }
    }
    ...
    public static void main(String[] argu) throws IOException {
        ...
        for (int id = 0; id < k; id++){
            ...
            ChessPiece tmp = new ChessPiece(x, y, id, dir);
                ...
                if (base.height == 1){
                    ...
                    if (bd.orderType != 2){
                        ...
                        for (int idx = 0; idx < size; idx++){
                            movePiece(chessLayer[baseY][baseX].get(idx), bd, idx);
                        }

                        chessLayer[baseY][baseX].clear();
                    }
                }
            }

            for (int id = 0; id < k; id++){
                if (chesspiece.get(id).height >= 4){
                    check = true;
                    break jump;
                }
            }
